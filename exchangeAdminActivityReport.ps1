<#
=============================================================================================
Name:           Audit Microsoft 365 Admin Activities using PowerShell 
Version:        1.0
...
4.Allows you to generate a admin activity report for a custom period.  
5.Automatically installs the EXO module (if not installed already) upon your confirmation. 
...
$OutputCSV=".\AdminActivityReport_$((Get-Date -format yyyy-MMM-dd-ddd` hh-mm` tt).ToString()).csv" 
...
Write-Host `nRetrieving admin activity log from $StartDate to $EndDate... -ForegroundColor Yellow
$Count=0
$ExportResult=""   
$ExportResults=@()  
while($true)
{ 
 ...
 if($AdminID -eq "")
{ 
 $AdminID=Read-Host Enter admin UPN '(eg:John@contoso.com)'
}
 #Getting audit log for given time range
 $Results=Search-UnifiedAuditLog -StartDate $CurrentStart -EndDate $CurrentEnd -UserIds $AdminID -SessionId s -SessionCommand ReturnLargeSet -ResultSize 5000
 ...
 foreach($Result in $Results)
 {
  ...
  $ExportResult=@{'Activity Time'=$ActivityTime;'Admin Name'=$UserID;'Operation'=$Operation;'Result'=$ResultStatus;'Workload'=$Workload;'More Info'=$MoreInfo}
  $ExportResults= New-Object PSObject -Property $ExportResult  
  $ExportResults | Select-Object 'Activity Time','Admin Name','Operation','Result','Workload','More Info' | Export-Csv -Path $OutputCSV -Notype -Append 
 }
